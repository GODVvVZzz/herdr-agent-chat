...
skill 和插件是同一个系统的两半——skill 教会 agent 协议,插件在 agent 无法行动时保证送达。两个都装(与 herdr-file-viewer、herdr-browser、herdr-annotate 相同的"插件+配套 skill"惯例):
...
插件可以在没有 skill 的情况下工作(只响应 session-chat manifest 里登记的 pane),skill 也可以在没有插件的情况下工作(worker 自己投递回信)——但两个一起装才是完整形态。
